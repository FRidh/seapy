...
class Junction(Base):
    ...
    def subsystems(self):
        ...
        """
        yield from (self.system.get_object(name) for name in unique(itertools.chain(*((coupling.subsystem_from.name, coupling.subsystem_to.name) for coupling in self.linked_couplings))))
    ...
    def add_coupling_manual(self, name, model, subsystem_from, subsystem_to, **properties):
        """
    ...
    def add_coupling(self, subsystem_from, subsystem_to, name=None, **properties):
        ...
        try:
            ...
            raise ValueError("No suitable model found for the combination of subsystems and junction shape.")
        if not name:
