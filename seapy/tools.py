    ...
    def energy_due_to_excitation(self, excitation=None):
        ...
        * an excitation object.
        ...
        if excitation is None:
            ...
            power = np.zeros(self._system.frequency.amount)
            for excitation in excitations:
                excitation = self._system.get_object(excitation)
                if excitation.subsystem == subsystem:
                    power += excitation.power
                else:
                    raise ValueError("Invalid excitation. The excitation {} is not connected to the first subsystem of this path, {}.".format(excitation.name, subsystem.name))
        else:
            excitation = self._system.get_object(excitation)
            if excitation.subsystem == subsystem:
                power = excitation.power
...
class PathAnalysis(object):
    ...
    def graph(self, objects=None):
        ...
        if 'subsystems' in objects and 'couplings' in objects:
            for coupling in system.couplings:
                G.add_path((coupling.subsystem_from.name, coupling.name, coupling.subsystem_to.name))
    ...
    def paths(self, subsystem_from, subsystem_to):
        ...
        yield from (Path(self._system, path) for path in nx.all_simple_paths(G, subsystem_from.name, subsystem_to.name))
